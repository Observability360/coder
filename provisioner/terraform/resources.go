...
func ConvertState(ctx context.Context, modules []*tfjson.StateModule, rawGraph string, logger slog.Logger) (*State, error) {
	...
	for _, resource := range sortedResources["coder_app"] {
		...
		// The browser navigates to external app URLs directly, so they must be
		// absolute. Terraform reports values it cannot resolve until apply as
		// empty, so only URLs it has actually resolved are validated here.
